...
规则：渲出成片的整条音轨相对视频有固定滞后，根因在输出编码/封装链路（AAC encoder priming，48kHz 典型 2048 samples ≈1.28f@30fps；HyperFrames 跟踪 heygen-com/hyperframes#7099），不是 `<Audio>`/`Sequence.from` 的行为——按渲染管线（HyperFrames 版本 + codec + sample rate + 容器）实测记档（渲出成片抽音轨，拿 2–3 个尖锐 SFX 原文件做归一化交叉相关，残差一致才算量准），补偿在渲染层的 from 计算式统一扣除，不逐条改帧号；音源自身的峰值滞后（峰值不在文件头）逐文件量。统一公式 `from = 目标峰值帧 − 峰值滞后 − 输出偏移`，方法与边界（截断型音效窗口不加长、片头 2f 内无法全补）见 sound-design §4.6。
...
自检：本管线的输出音轨偏移实测过吗（HyperFrames 版本/codec/采样率/容器随值记录在哪）？补偿是 from 计算式统一扣的还是散落在各条帧号里？带明显攻击峰的音源，峰值滞后量过了吗？
